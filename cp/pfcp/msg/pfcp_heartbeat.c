...
process_heartbeat_response(uint8_t *buf_rx, struct sockaddr_in *peer_addr)
{
	...
	if (ret == -1) {
		...
		if(recov_time == 0) {
		    LOG_MSG(LOG_INFO, "Received heartbeat response from %s, received recovery time %u, old recovery time %u ", inet_ntoa(peer_addr->sin_addr), update_recov_time, recov_time);
