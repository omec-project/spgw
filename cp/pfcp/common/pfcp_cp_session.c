	...
	if ((cp_config->cp_type == PGWC) || (cp_config->cp_type == SAEGWC))
	{
        ...
        while (pcc_rule != NULL) {
            ...
            if(cp_config->gx_enabled) {
                ...
                //if (bearer->num_dynamic_filters > 1 && bearer->ambr_qer_flag == false) {
                if (bearer->ambr_qer_flag == false) {
                    ...
                    fill_qer_entry(pdn, bearer, bearer->qer_count, true);
                    bearer->qer_count++;
			...
			fill_qer_entry(pdn, bearer, bearer->qer_count, false);
            bearer->qer_count++;
			...
			bearer->dynamic_rules[bearer->num_dynamic_filters]->pdr[0] = temp_pdr; 
            bearer->pdr_count++;
		    ...
		    fill_sdf_rules(pfcp_sess_est_req, pcc_rule->dyn_rule, bearer->pdr_count);
			...
			bearer->dynamic_rules[bearer->num_dynamic_filters]->pdr[1] = temp_pdr; 
            bearer->pdr_count++;

			f_status = (enum flow_status)bearer->dynamic_rules[bearer->num_dynamic_filters]->flow_status; 
	...
	//}
	{
		...
		for(uint8_t i = 0; i < MAX_BEARERS; i++)
		{
            ...
            for(uint8_t idx = 0; idx < bearer->pdr_count; idx++)
			{
				...
				pfcp_sess_est_req->create_pdr[pdr_idx].pdr_id.rule_id  = bearer->pdrs[idx]->rule_id;
				pfcp_sess_est_req->create_pdr[pdr_idx].precedence.prcdnc_val = bearer->pdrs[idx]->prcdnc_val;
				...
				if (cp_config->cp_type == PGWC || cp_config->cp_type == SAEGWC){
                    LOG_MSG(LOG_DEBUG, "Number of QERs on PDR %d are %d ", bearer->pdrs[idx]->rule_id, bearer->pdrs[idx]->qer_id_count);
					pfcp_sess_est_req->create_pdr[pdr_idx].qer_id_count = bearer->pdrs[idx]->qer_id_count;
					for(int itr1 = 0; itr1 < bearer->pdrs[idx]->qer_id_count; itr1++) {
						pfcp_sess_est_req->create_pdr[pdr_idx].qer_id[itr1].qer_id_value = bearer->pdrs[idx]->qer_id[itr1].qer_id;
                        LOG_MSG(LOG_DEBUG,"Adding QER %d in PDR-ID %d ", bearer->pdrs[idx]->qer_id[itr1].qer_id, bearer->pdrs[idx]->rule_id);
					}
				}
                ...
                if (cp_config->cp_type == PGWC || cp_config->cp_type == SAEGWC) {
                    if(cp_config->urr_enable == 1) {
                        LOG_MSG(LOG_DEBUG,"Number of URRs on PDR (%d)  are %d ", bearer->pdrs[idx]->rule_id, bearer->pdrs[idx]->urr_id_count);
                        ...
                        for(int itr1 = 0; itr1 < bearer->pdrs[idx]->urr_id_count; itr1++) {
                            pfcp_sess_est_req->create_pdr[pdr_idx].urr_id[itr1].urr_id_value = bearer->pdrs[idx]->urr_id[itr1].urr_id;
                            LOG_MSG(LOG_DEBUG,"Adding URR %d in PDR-ID %d ", bearer->pdrs[idx]->urr_id[itr1].urr_id, bearer->pdrs[itr1]->rule_id);
                        }
                    }
                }
		...
		ue_context_t *context, pdn_connection_t *pdn)
{
    ...
    if(cp_config->gx_enabled) {
        if (cp_config->cp_type != SGWC){
            bearer->qer_count = NUMBER_OF_QER_PER_BEARER; // FIXME 
            for(uint8_t itr=0; itr < bearer->qer_count; itr++){
                bearer->qer_id[itr].qer_id = generate_qer_id(SOURCE_INTERFACE_VALUE_CORE); // FIXME
                ...
                fill_qer_entry(pdn, bearer, itr, false);
            }
            ...
            for(uint8_t itr=0; itr < bearer->urr_count; itr++) {
			    bearer->urr_id[itr].urr_id = generate_urr_id(SOURCE_INTERFACE_VALUE_CORE); // FIXME
            }
        ...
        for(uint8_t itr=0; itr < bearer->pdr_count; itr++){
            switch(itr) {
                case SOURCE_INTERFACE_VALUE_ACCESS:
                    fill_pdr_entry(context, pdn, bearer, SOURCE_INTERFACE_VALUE_ACCESS, itr, 0, 0); // FIXME
                    break;
                case SOURCE_INTERFACE_VALUE_CORE:
                    fill_pdr_entry(context, pdn, bearer, SOURCE_INTERFACE_VALUE_CORE, itr, 0, 0); // FIXME
                    break;
                default:
                    break;
            }
