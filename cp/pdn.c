					 ...
					 bearer_qos_ie *rule_qos)
{
	...
	if(default_bearer_qos->arp.preemption_vulnerability != rule_qos->arp.preemption_vulnerability) {
        LOG_MSG(LOG_DEBUG," arp preemption vulnerability is not matched %d %d ", default_bearer_qos->arp.preemption_vulnerability, rule_qos->arp.preemption_vulnerability);
		return -1;
    }

	if(default_bearer_qos->arp.priority_level != rule_qos->arp.priority_level) {
        LOG_MSG(LOG_DEBUG," arp priority level is not matched %d %d ", default_bearer_qos->arp.priority_level, rule_qos->arp.priority_level);
		return -1;
    }
